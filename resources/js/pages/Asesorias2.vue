...
/**
 * Datos reactivos (desde props del backend)
 */
...
watch(semester, (newVal) => {
    if (newVal !== props.currentSemester) {
        router.get('/asesorias2', { semester: newVal }, { preserveState: true });
    }
});
 ...
 */
const expandedFollowupKey = ref(null); // column key | null
...
function exportCSV() {
    ...
    FOLLOWUPS.value.forEach(f => {
        headers.push(f.label.toUpperCase() + '_STATUS');
        headers.push(f.label.toUpperCase() + '_PCT');
    });
    headers.push('REPORTES_DOCS', 'REPORTES_PCT', 'ESTADO_FINAL', 'SEMESTRE');
    ...
    const lines = [
        ...
        ...filteredRows.value.map((r) => {
            ...
            FOLLOWUPS.value.forEach(f => {
                values.push(followupStatus(r, f.key));
                values.push(String(checklistProgress(r, f.key).pct));
            });
    ...
    downloadBlob(blob, `seguimiento_detallado_${semester.value || 'todos'}.csv`);
}
...
function exportXLSX() {
    const data = filteredRows.value.map((r) => {
        ...
        FOLLOWUPS.value.forEach(f => {
            row[f.label.toUpperCase() + '_STATUS'] = followupStatus(r, f.key);
            row[f.label.toUpperCase() + '_PCT'] = checklistProgress(r, f.key).pct;
        });

        row.REPORTES_DOCS = docsCount;
        row.REPORTES_PCT = reportsPct(r);
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Asesorias_Detalle');
    ...
    downloadBlob(blob, `seguimiento_detallado_${semester.value || 'todos'}.xlsx`);
}
...
const followupModalProgress = computed(() => {
    if (!followupModalRow.value || !followupModalKey.value)
...
<template>
    <AppLayout :breadcrumbs="[{ title: 'Asesorías 2', href: '/asesorias2' }]">
            ...
            <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
                ...
                <div class="mb-4">
                    ...
                    <p class="mt-1 text-sm text-gray-600">
                        Vista preliminar (sin base de datos). Estados: NA / OK /
                        NE.
                    </p>

                    <div
                        v-if="expandedFollowupMeta"
                        class="mt-2 text-xs text-gray-600"
                    >
                        Mostrando columnas extra de:
                        <span class="font-semibold">{{
                            expandedFollowupMeta.label
                        }}</span>
                        <button type="button" class="ml-2 rounded-lg border border-gray-200 bg-white px-2 py-1 text-xs font-semibold text-gray-700 hover:bg-gray-50"
                ...
                >
                    <div class="flex flex-1 items-center gap-3">
                        <div class="relative w-full max-w-2xl">
                            <input
                                v-model="search"
                                type="text"
                                class="w-full rounded-lg border border-gray-200 bg-white px-4 py-2 pl-10 text-sm ring-0 outline-none focus:border-gray-300"
                                placeholder="Buscar por maestro, materia o clave..."
                            />
                            <div
                                class="pointer-events-none absolute top-2.5 left-3 text-gray-400"
                            >
                                <svg
                                    width="18"
                                    height="18"
                        ...
                        <div class="flex items-center gap-2">
                            <span class="hidden text-sm text-gray-500 md:inline"
                                >Semestre</span
                            >
                            <select
                                v-model="semester"
                    ...
                    <div class="relative flex items-center gap-2">
                        ...
                        <div
                            v-if="exportMenuOpen"
                            class="absolute top-11 right-0 z-20 w-44 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-lg"
                            role="menu"
                        >
                ...
                >
                    <div class="overflow-x-auto">
                        <table
                            ref="printTableRef"
                            class="w-full min-w-[1100px] table-auto"
                        >
                            <thead class="bg-gray-50">
                                <tr
                                    class="text-left text-xs font-semibold tracking-wide text-gray-600 uppercase"
                                >
                                    ...
                                    <th
                                        ...
                                        :key="f.key"
                                    >
                                        <button type="button" class="inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-2 py-1 text-[11px] font-semibold text-gray-700 hover:bg-gray-50"
                                            @click="toggleExpandFollowup(f.key)"
                                            :title="
                                                expandedFollowupKey === f.key
                                                    ? 'Cerrar expansión'
                                                    : 'Expandir columnas a la derecha'
                                            "
                                        >
                                            {{ f.label }}
                                    ...
                                    <template v-if="expandedFollowupMeta">
                                        <th
                                            class="bg-indigo-50 px-4 py-3 text-indigo-700"
                                        >
                                            {{ expandedFollowupMeta.label }} %
                                            realizado
                                        </th>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                >
                                    ...
                                    <td
                                        class="px-4 py-4 font-mono text-sm text-gray-700"
                                    >
                                        {{ row.clave_tecnm }}
                                    </td>
                                    ...
                                    >
                                        <div class="flex items-center gap-2">
                                            ...
                                            <button type="button" class="rounded-lg border border-gray-200 bg-white px-2 py-1 text-xs font-semibold text-gray-700 hover:bg-gray-50"
                                                :title="
                                                    expandedFollowupKey ===
                                                    f.key
                                                        ? 'Cerrar expansión'
                                                        : 'Expandir columnas del seguimiento'
                                                "
                                                @click="
                                                    toggleExpandFollowup(f.key)
                                                "
                                            >
                                                {{
                                                    expandedFollowupKey ===
                                    ...
                                    <template v-if="expandedFollowupMeta">
                                        <td class="bg-indigo-50/40 px-4 py-4">
                                            <span
                                                class="inline-flex rounded-full border border-indigo-200 bg-white px-3 py-1 text-xs font-semibold text-indigo-700"
                                            >
                                                {{
                                                    checklistProgress(
                                                        row,
                                                        expandedFollowupMeta.key,
                                                    ).pct
                                                }}%
                                            </span>
                                        </td>
                                        ...
                                        <td class="bg-indigo-50/40 px-4 py-4">
                                            <div
                                                class="max-w-[260px] truncate text-sm text-gray-700"
                                            >
                                                {{
                                                    ensureFollowup(
                                                        row,
                                        ...
                                        <td class="bg-indigo-50/40 px-4 py-4">
                                            <button type="button" class="rounded-lg border border-indigo-200 bg-white px-3 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-50"
                                                @click="
                                                    openFollowupDetail(
                                                        row,
                                                        expandedFollowupMeta.key,
                ...
                <div
                    v-if="docsModalOpen"
                    class="modal-layer fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"
                ...
                >
                    <div class="w-full max-w-lg rounded-xl bg-white shadow-lg">
                        ...
                        >
                            <div>
                                ...
                                <p class="text-sm text-gray-600">
                                    {{ docsModalRow?.maestro }} —
                                    {{ docsModalRow?.materia }}
                                </p>
                            </div>
                        ...
                        <div class="px-5 py-4">
                            <div
                                v-if="
                                    (docsModalRow?.reportes_docs?.length ??
                                        0) === 0
                                "
                            ...
                            <ul v-else class="space-y-2">
                                ...
                                >
                                    ...
                                    <button type="button" class="rounded-lg border border-gray-200 bg-white px-3 py-1 text-xs font-semibold text-gray-700 hover:bg-gray-50"
                                        @click="
                                            alert(
                                                'Sin BD por ahora: aquí luego abrirás/descargarás el documento.',
                                            )
                                        "
                                    >
                                        Abrir
                    ...
                    <div class="w-full max-w-xl rounded-xl bg-white shadow-lg">
                        ...
                        >
                            <div>
                                ...
                                <p class="text-sm text-gray-600">
                                    {{ viewModalRow?.maestro }} —
                                    {{ viewModalRow?.materia }}
                                </p>
                            </div>
                            <button type="button" class="rounded-lg p-2 text-gray-500 hover:bg-gray-100"
                                @click="closeView"
                        ...
                        <div
                            class="grid grid-cols-1 gap-4 px-5 py-4 sm:grid-cols-2"
                        >
                            ...
                            <div class="rounded-lg border border-gray-100 p-3">
                                ...
                                >
                                    {{ viewModalRow?.clave_tecnm }}
                                </div>
                            </div>

                            <div v-for="f in FOLLOWUPS" :key="f.key" class="rounded-lg border border-gray-100 p-3">
                                ...
                                <div class="mt-1 text-sm">
                                    <span
                                        ...
                                        :class="statusPillClasses(followupStatus(viewModalRow, f.key))"
                                    >
                                        {{ followupStatus(viewModalRow, f.key) }}
                                        ({{ checklistProgress(viewModalRow, f.key).pct }}%)
                                    </span>
                                </div>
                            </div>
                            ...
                            <div
                                class="rounded-lg border border-gray-100 p-3 sm:col-span-2"
                            >
                                <div
                                    class="text-xs font-semibold text-gray-500"
                                >
                                ...
                                <div class="mt-1 text-sm text-gray-900">
                                    {{
                                        viewModalRow?.reportes_docs?.length ?? 0
                                    }}
                                    documentos ({{ reportsPct(viewModalRow) }}%)
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="w-full max-w-2xl rounded-xl bg-white shadow-lg">
                        ...
                        >
                            <div>
                                ...
                                <p class="text-sm text-gray-600">
                                    {{ followupModalRow?.maestro }} —
                                    {{ followupModalRow?.materia }}
                                </p>
                                <p class="mt-1 text-xs text-gray-500">
                                    Progreso: {{ followupModalProgress.pct }}%
                                    ({{ followupModalProgress.done }}/{{
                                        followupModalProgress.total
                                    }})
                                </p>
                            </div>
                            <button type="button" class="rounded-lg p-2 text-gray-500 hover:bg-gray-100"
                        ...
                        <div
                            class="grid grid-cols-1 gap-4 px-5 py-4 md:grid-cols-2"
                        >
                            <!-- Checklist -->
                            <div class="rounded-lg border border-gray-100 p-4">
                                ...
                                <div
                                    v-if="
                                        (followupModalData?.checklist?.length ??
                                            0) === 0
                                    "
                                ...
                                >
                                    ...
                                    <ul v-else class="mt-3 space-y-2">
                                        <li
                                            v-for="(
                                                e, idx
                                            ) in followupModalData.evidencias"
                                            :key="idx"
                                            class="flex items-center justify-between rounded-lg border border-gray-100 px-3 py-2"
                                        >
                                            <div class="min-w-0">
                                            ...
                                            <button type="button" class="rounded-lg border border-gray-200 bg-white px-3 py-1 text-xs font-semibold text-gray-700 hover:bg-gray-50"
                                                @click="
                                                    alert(
                                                        'Sin BD por ahora: aquí luego abrirás/descargarás la evidencia.',
                                                    )
                                                "
                                            >
                                                Abrir
                ...
                <div class="mt-4 text-xs text-gray-500">
                    Tip: click en los chips (NA/OK/NE) para alternar estados.
                    Click en SEG 01/SEG 02 para expandir columnas.
                </div>
            </div>
        </div>
    </AppLayout>
