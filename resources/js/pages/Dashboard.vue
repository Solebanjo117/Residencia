...
const { isDocente, isJefeOficina, isJefeDepto } = useAuth();
...
const roleMessage = computed(() => {
    if (isDocente.value)
        return 'Monitorea tus entregas y responde a tiempo las ventanas activas.';
    if (isJefeOficina.value)
        return 'Prioriza revisiones pendientes y mantén continuidad del dictamen institucional.';
    if (isJefeDepto.value)
        return 'Controla configuración de ventanas y cobertura de requerimientos por departamento.';
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto flex max-w-7xl flex-col gap-6 px-6 py-8">
            <section
                class="overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-700 p-6 text-white shadow-sm"
            >
                <div
                ...
                >
                    <div>
                        <div
                            class="mb-3 inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-semibold tracking-wide uppercase"
                        >
                            <ShieldCheck class="h-4 w-4" />
                            {{ roleLabel }}
                        </div>
                        <h1
                            class="text-2xl leading-tight font-bold md:text-3xl"
                        >
                            Panel Institucional ASAD
                        </h1>
                    ...
                    <div
                        class="rounded-xl border border-white/20 bg-white/10 p-4 text-sm backdrop-blur"
                    >
                        <div class="text-slate-200">Semestre activo</div>
                        <div class="mt-1 text-lg font-semibold">
                            {{ props.semester?.name ?? 'No configurado' }}
                        </div>
                    </div>
                </div>
            </section>
            ...
            >
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm">
                            Puedes avanzar con configuración base en semestres,
                            ventanas y requerimientos antes de operar el flujo
                            completo.
                        </p>
                    </div>
                </div>
            ...
            <section
                class="grid gap-4 md:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-6"
            >
                <article
                ...
                >
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <component
                            :is="metricIcon[metric.key] ?? FileStack"
                            class="h-4 w-4"
                        />
                    </div>
                    <div class="mt-3 text-3xl leading-none font-bold">
                        {{ metric.value }}
                    </div>
            ...
            <section class="grid gap-6 lg:grid-cols-2">
                    ...
                    <div class="grid gap-3">
                        ...
                        >
                            ...
                            >
                                ...
                                <ArrowUpRight
                                    class="h-4 w-4 text-slate-500 transition group-hover:text-slate-700"
                                />
                            </div>
                        </Link>
                    </div>
                ...
                >
                    ...
                    <div
                        v-if="props.upcomingDeadlines.length === 0"
                        class="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-6 text-sm text-slate-500"
                    >
                        No hay ventanas activas o programadas para mostrar en
                        este momento.
                    </div>

                    <ul v-else class="space-y-3">
                        <li
                            v-for="deadline in props.upcomingDeadlines"
                            :key="deadline.id"
                        ...
                        >
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p class="mt-1 text-xs text-slate-500">
                                        Apertura:
                                        {{ formatDate(deadline.opens_at) }}
                                    </p>
                                    <p class="text-xs text-slate-500">
                                        Cierre:
                                        {{ formatDate(deadline.closes_at) }}
                                    </p>
                                </div>
                                <span
                            ...
                            <p class="mt-2 text-xs font-medium text-slate-700">
                                {{ daysRemaining(deadline.closes_at) }}
                            </p>
                        </li>
                    </ul>
                </div>
