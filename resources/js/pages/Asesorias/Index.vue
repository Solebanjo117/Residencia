...
body{padding-bottom:42mm}
h1{font-size:18px;margin:0 0 4px;text-align:center}
p{font-size:12px;margin:0 0 12px;color:#444;text-align:center}
table{width:100%;border-collapse:collapse;margin-top:8px}
th,td{border:1px solid #d1d5db;padding:6px 10px;font-size:11px;vertical-align:middle}
...
.signature-block{width:32%;text-align:center}
.signature-line{border-top:1px solid #111;padding-top:6px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.04em}
</style></head><body>
...
<div class="signature-footer">
    <div class="signature-block">
        <div class="signature-line">Academia</div>
    </div>
    <div class="signature-block">
        <div class="signature-line">Jefe de Departamento</div>
    </div>
</div>
</body></html>`);
...
<template>
    <AppLayout :breadcrumbs="[{ title: 'Asesorias - Horarios', href: '/asesorias-horarios' }]">
        <div class="min-h-screen bg-gray-50">
            <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
                <div class="mb-4">
                    <h1 class="text-2xl font-semibold text-gray-900">Horario de Asesorias</h1>
                    <p class="mt-1 text-sm text-gray-600">Horarios de asesoria de todos los docentes por materia.</p>
                </div>

                <div class="toolbar mb-4 flex flex-col gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm md:flex-row md:items-center md:justify-between">
                        ...
                        <table ref="printTableRef" class="w-full table-auto">
                            <thead class="bg-gray-50">
                                <tr class="text-[11px] font-semibold uppercase tracking-wide text-gray-600">
                                    <th class="min-w-[180px] px-4 py-3 text-left">Materia</th>
                                    <th class="min-w-[150px] px-4 py-3 text-left">Docente</th>
                                    <th v-for="day in dayCols" :key="day.key" class="min-w-[100px] px-3 py-3 text-center">
                                        {{ day.label }}
                                    </th>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <tr v-for="row in filteredRows" :key="row.id" class="hover:bg-gray-50/60">
                                    ...
                                    <td v-for="day in dayCols" :key="day.key" class="day-cell px-3 py-3 text-center">
                                        <span
                                            v-if="row[day.key]"
