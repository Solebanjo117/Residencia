...
const canOfficeReview = computed(() => isAdminAuthority.value);
const canFinalApprove = computed(() => isAdminAuthority.value);
const canManageApplicability = computed(() => isAdminAuthority.value);
...
function statusTooltip(cell) {
    ...
    const base = {
        VF: 'Liberado con visto bueno final',
        AO: 'Aprobado, pendiente de liberacion final',
...
function availabilityToneClasses(code) {
    ...
    if (code === 'UPCOMING' || code === 'STAGE_LOCKED')
                ...
                <div class="space-y-4 px-5 py-4">
                    ...
                    <div
                        v-if="
                            ...
                            cellModalData.can_mark_na &&
                            cellModalData.status !== 'NA' &&
                            !['AO', 'VF'].includes(cellModalData.status)
                        "
                    ...
                    >
                        ...
                        <div class="mt-3 flex items-center gap-2">
                            <button
                                ...
                                @click="updateCellStatus('NA')"
                            >
                                Marcar NA
                            </button>
                            <button
                                v-if="cellModalData.can_reactivate"
                                type="button"
                                class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50"
                                :disabled="cellStatusForm.processing"
                                @click="updateCellStatus('DRAFT')"
                            >
                                Reactivar
                            </button>
                        </div>
