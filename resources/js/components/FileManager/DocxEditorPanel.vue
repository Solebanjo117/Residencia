...
const props = withDefaults(
    defineProps<{
        ...
        backLabel?: string;
        shellClass?: string;
        allowUnsafeRewrite?: boolean;
        unsafeRewriteNotice?: string;
    }>(),
    {
        heading: 'Editor DOCX',
        description:
        ...
        shellClass: '',
        allowUnsafeRewrite: false,
        unsafeRewriteNotice:
            'Este DOCX contiene estructura avanzada de Word. Puedes guardarlo porque estas editando un archivo propio habilitado para cambios, pero algunos elementos no soportados pueden simplificarse.',
    },
);
...
const unsafeRewriteAllowed = computed(
    () =>
        props.allowUnsafeRewrite &&
        props.capabilities.can_edit &&
        !props.document.safe_to_save &&
        !props.document.load_error,
);
const docxSaveError = computed(
    () => (saveForm.errors as Record<string, string | undefined>).docx,
);
...
function insertLink() {
    ...
    if (selection && !selection.isCollapsed) {
        ...
        const label = window.prompt('Texto del enlace', trimmedUrl) || trimmedUrl;
        document.execCommand(
            'insertHTML',
            false,
    ...
    <div :class="shellClass">
        ...
        <div class="mb-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
            <div class="space-y-4">
                ...
                <div
                    v-if="!document.safe_to_save && !document.load_error"
                    class="rounded-2xl border border-sky-200 bg-sky-50 p-4 text-sm text-sky-900 shadow-sm"
                >
                    <div class="mb-2 flex items-center gap-2 font-semibold">
                        <AlertTriangle class="h-4 w-4" />
                        {{
                            unsafeRewriteAllowed
                                ? 'Advertencia de compatibilidad'
                                : 'Documento protegido contra cambios de estructura'
                        }}
                    </div>
                    <p v-if="unsafeRewriteAllowed">
                        {{ unsafeRewriteNotice }}
                    </p>
                    <p v-else>
                        Este archivo se puede visualizar y descargar, pero el guardado desde el
                        editor web esta desactivado para evitar que Word cambie tablas,
                        secciones, comentarios, campos u otros elementos avanzados.
                    </p>
                    <ul v-if="document.blocking_features.length > 0" class="mt-3 space-y-1">
                        <li
                ...
                >
                    ...
                    <p class="mt-2 text-rose-700">
                        Puedes descargar el archivo original o reemplazarlo desde el gestor. No se
                        modifico nada automaticamente.
                    </p>
                </div>
                ...
                <div v-else class="rounded-2xl border border-slate-200 bg-white shadow-sm">
                        ...
                        <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
                            <div>
                                ...
                                <p class="mt-1 text-xs text-slate-500">
                                    Edicion textual por secciones. El toolbar actua sobre la seccion
                                    que tenga el cursor activo.
                                </p>
                            </div>
                            <div
                                v-if="document.stats"
                            ...
                            >
                                <span class="rounded-full border border-slate-200 bg-slate-50 px-2 py-1">
                                    Parrafos: {{ document.stats.paragraphs }}
                                </span>
                                <span class="rounded-full border border-slate-200 bg-slate-50 px-2 py-1">
                                    Titulos: {{ document.stats.headings }}
                                </span>
                                <span class="rounded-full border border-slate-200 bg-slate-50 px-2 py-1">
                                    Listas: {{ document.stats.list_items }}
                                </span>
                                <span class="rounded-full border border-slate-200 bg-slate-50 px-2 py-1">
                                    Imagenes: {{ document.stats.images }}
                                </span>
                                <span class="rounded-full border border-slate-200 bg-slate-50 px-2 py-1">
                                    Tablas: {{ document.stats.tables }}
                                </span>
                            </div>
                        </div>
                        ...
                        >
                            <div class="mb-2 flex items-center justify-between gap-3">
                                <div>
                                    ...
                                    <p class="mt-1 text-xs text-slate-500">
                                        Edita el encabezado actual del documento.
                                    </p>
                                </div>
                                <span class="rounded-full border border-slate-200 bg-white px-2 py-1 text-[11px] font-semibold text-slate-600">
                                    Seccion
                                </span>
                            </div>
                            <div
                        ...
                        >
                            <div class="mb-2 flex items-center justify-between gap-3">
                                <div>
                                    ...
                                    <p class="mt-1 text-xs text-slate-500">
                                        Edita el pie de pagina actual del documento.
                                    </p>
                                </div>
                                <span class="rounded-full border border-slate-200 bg-white px-2 py-1 text-[11px] font-semibold text-slate-600">
                                    Seccion
                    ...
                    <div class="border-t border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-500">
                        <span v-if="readOnly">
                            Este documento esta en modo solo lectura. Puedes descargarlo o volver al
                            gestor.
                        </span>
                        <span v-else>
                            Guardar crea una revision segura del DOCX. Se intenta conservar la
                            tipografia explicita, la alineacion del parrafo, las imagenes incrustadas,
                            las listas simples, las tablas basicas y, cuando existen, el encabezado y
                            pie de pagina editables.
                        </span>
                    </div>
                </div>
            </div>
